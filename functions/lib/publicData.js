...
const HERO_BANNER_SELECT = "id,title,subtitle,image_url,link,button_text,brand_text,banner_type,tab_id,display_order,is_active,created_at,target_product_code";
const POPUP_SELECT = "id,title,image_url,link,start_date,end_date,display_order,is_active,created_at,target_product_code";
const PRODUCT_SUMMARY_SELECT = "id,product_code,name,category,display_order,external_link_url,price,description,short_description,image_url,stock,created_at,product_type";
const PRODUCT_DETAIL_SELECT = "id,name,category,_parent_category,display_order,external_link_url,price,description,short_description,image_url,stock,discount_rate,rating,review_count,created_at,product_type,basic_components,additional_components,cooperative_components,place_components,food_components";
...
const NOTICE_SUMMARY_SELECT = "id,title,excerpt,image_url,published_at,category,display_order,is_active,created_at,updated_at";
...
const dedupeCategories = (categories) => Array.from(new Set(categories
...
const toOptionalString = (value) => (typeof value === "string" ? value : undefined);
...
const sortNoticeSummaries = (items) => [...items].sort((a, b) => {
    const aOrder = toNumber(a.display_order, Number.MAX_SAFE_INTEGER);
    const bOrder = toNumber(b.display_order, Number.MAX_SAFE_INTEGER);
    if (aOrder !== bOrder)
        return aOrder - bOrder;
...
const sortFaqRows = (items) => [...items].sort((a, b) => {
    const aOrder = toNumber(a.display_order, Number.MAX_SAFE_INTEGER);
    const bOrder = toNumber(b.display_order, Number.MAX_SAFE_INTEGER);
    if (aOrder !== bOrder)
        return aOrder - bOrder;
...
const buildSupabaseUrl = (table, options) => {
...
const fetchSupabaseRows = async (table, options = {}) => {
...
const safeFetchSupabaseRows = async (table, options = {}, fallback = [], { quietOnSchemaError = false } = {}) => {
    try {
...
const fetchNavMenuRows = async (options = {}) => {
    try {
        return await fetchSupabaseRows("nav_menu_items", Object.assign(Object.assign({}, options), { select: NAV_ITEM_SELECT }));
    }
    catch (error) {
        if (isIgnorableSchemaError(error)) {
            return fetchSupabaseRows("nav_menu_items", Object.assign(Object.assign({}, options), { select: LEGACY_NAV_ITEM_SELECT }));
        }
        throw error;
    }
};
const fetchInstallationCaseRows = async (options = {}) => {
    try {
        return await fetchSupabaseRows("installation_cases", Object.assign(Object.assign({}, options), { select: CASE_SUMMARY_SELECT }));
    }
    catch (error) {
        if (isIgnorableSchemaError(error)) {
            return fetchSupabaseRows("installation_cases", Object.assign(Object.assign({}, options), { select: LEGACY_CASE_SUMMARY_SELECT }));
        }
        throw error;
    }
};
const fetchInstallationCaseDetailRows = async (options = {}) => {
    try {
        return await fetchSupabaseRows("installation_cases", Object.assign(Object.assign({}, options), { select: CASE_DETAIL_SELECT }));
    }
    catch (error) {
        if (isIgnorableSchemaError(error)) {
            return fetchSupabaseRows("installation_cases", Object.assign(Object.assign({}, options), { select: LEGACY_CASE_DETAIL_SELECT }));
        }
        throw error;
    }
};
const fetchInstallationCaseCategories = async () => {
    const rows = await safeFetchSupabaseRows("page_contents", {
        select: "content",
        filters: {
            page_key: `eq.${INSTALLATION_CASE_CATEGORIES_PAGE_KEY}`,
        },
        limit: 1,
    }, [], { quietOnSchemaError: true });
    const entry = rows[0];
    if (!isRecord(entry) || !isRecord(entry.content) || !Array.isArray(entry.content.categories)) {
        return [];
    }
    return dedupeCategories(entry.content.categories.filter((category) => typeof category === "string"));
};
const getCachedPayload = async (key, loader) => {
...
const getRouteSegments = (rawPath) => getNormalizedRoutePath(rawPath)
    .replace(/^\/+|\/+$/g, "")
    .split("/")
    .filter(Boolean);
const createNoticeSummary = (item) => ({
...
const loadBootstrapPayload = async () => {
    const [navItems, installationCaseCategories, publicVisualEntries] = await Promise.all([
        fetchNavMenuRows(),
        fetchInstallationCaseCategories(),
...
const loadHomePayload = async () => {
    ...
    const companyIntroImageUrl = typeof (companyOverview === null || companyOverview === void 0 ? void 0 : companyOverview.imageUrl) === "string" && companyOverview.imageUrl.trim()
...
const loadProductDetailPayload = async (id) => {
...
const loadCaseDetailPayload = async (id) => {
...
const loadNoticeDetailPayload = async (id) => {
    const [detailRows, summaryRows] = await Promise.all([
        safeFetchSupabaseRows("notice_posts", {
    ...
    return {
        generatedAt: new Date().toISOString(),
        post: detailRows[0]
            ? Object.assign(Object.assign({}, detailRows[0]), { imageUrl: toStringValue(detailRows[0].image_url), publishedAt: toIsoDate(detailRows[0].published_at), contentHtml: toStringValue(detailRows[0].content_html), displayOrder: toNumber(detailRows[0].display_order), isActive: detailRows[0].is_active !== false, created_at: toOptionalString(detailRows[0].created_at), updated_at: toOptionalString(detailRows[0].updated_at) }) : null,
        previousNotice: currentIndex > 0 ? orderedNotices[currentIndex - 1] : null,
        nextNotice: currentIndex >= 0 && currentIndex < orderedNotices.length - 1
...
const loadCompanyPayload = async () => {
    ...
    return {
        generatedAt: new Date().toISOString(),
        content: isRecord(rows[0]) ? (_a = rows[0].content) !== null && _a !== void 0 ? _a : {} : {},
    };
};
const loadPayloadForPath = async (path) => {
...
const handlePublicDataRequest = async (req, res) => {
    if (req.method !== "GET") {
...
exports.handlePublicDataRequest = handlePublicDataRequest;

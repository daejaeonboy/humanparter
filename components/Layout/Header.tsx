...
import { getPublicBootstrapData, subscribePublicDataInvalidation } from "../../src/api/publicDataApi";
import {
...
export const Header: React.FC = () => {
  ...
  const preloadedInstallationCaseCategories = usePrerenderData()?.bootstrap?.installationCaseCategories;
  ...
  const [installationCaseCategories, setInstallationCaseCategories] = useState<string[]>(
    preloadedInstallationCaseCategories || [],
  );
  ...
  const megaMenuLookup = useMemo(
    () => buildPublicMegaMenuItems(publicVisuals, installationCaseCategories),
    [installationCaseCategories, publicVisuals],
  );
  ...
  useEffect(() => {
    ...
    const applyBootstrapData = (
      ...
      nextInstallationCaseCategories: string[] | undefined,
      ...
      if (nextInstallationCaseCategories) {
        setInstallationCaseCategories(nextInstallationCaseCategories);
      }
    ...
    const loadMegaMenuItems = async () => {
      try {
        const { navItems, installationCaseCategories: nextInstallationCaseCategories, publicVisuals: nextPublicVisuals } = await getPublicBootstrapData();
        applyBootstrapData(navItems, nextInstallationCaseCategories, nextPublicVisuals);
      } catch (error) {
        console.error("Failed to load mega menu items:", error);
      }
    };

    applyBootstrapData(preloadedNavItems, preloadedInstallationCaseCategories, preloadedPublicVisuals);
  ...
  }, [preloadedInstallationCaseCategories, preloadedNavItems, preloadedPublicVisuals]);

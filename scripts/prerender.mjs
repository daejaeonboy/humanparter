...
const dedupeCategories = (categories) =>
  Array.from(new Set(categories.map((category) => normalizeCategoryLabel(category)).filter(Boolean)));
...
const loadDatasets = async () => {
  ...
  const [
    ...
    companyEntries,
    publicVisualEntries,
    installationCaseCategoryEntries,
  ] = await Promise.all([
  ...
  const installationCaseCategories = Array.isArray(installationCaseCategoryEntries?.[0]?.content?.categories)
    ? dedupeCategories(
        installationCaseCategoryEntries[0].content.categories.filter((category) => typeof category === 'string'),
      )
    : [];
...
const buildPrerenderEntries = (datasets) => {
  const baseData = datasets
    ? {
        ...
        bootstrap: {
          ...
          installationCaseCategories: datasets.installationCaseCategories,
  ...
  setRouteData('/cases', {
    installationCases: {
      generatedAt: datasets.generatedAt,
      categories: datasets.installationCaseCategories,
      cases: caseSummaries,
    },
  });

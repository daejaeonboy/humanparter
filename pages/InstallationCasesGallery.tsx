...
import { getPublicCasesData, invalidatePublicDataCache, subscribePublicDataInvalidation, type PublicCaseSummary } from '../src/api/publicDataApi';
import { getInstallationCaseTabValue } from '../src/config/publicMegaMenu';
...
const DEFAULT_INSTALLATION_CASE_CATEGORY = '임시사무실';

const normalizeCategoryLabel = (value?: string) => value?.trim().replace(/\s+/g, ' ') || '';

const toCategoryTabValue = (category: string) => {
  const normalized = normalizeCategoryLabel(category);
  if (!normalized) return '';
  if (normalized === '임시사무실') return 'temporary-office';
  if (normalized === '공공기관') return 'public-institution';
...
export const InstallationCasesGallery: React.FC = () => {
  ...
  const preloadedCategories = prerenderData?.installationCases?.categories;
  ...
  const [managedCategories, setManagedCategories] = useState<string[]>(preloadedCategories || []);
  ...
  const filterCategoryOptions = useMemo(
    () =>
      Array.from(
        new Set(
          [...managedCategories.map((item) => normalizeCategoryLabel(item)), ...categoryOptions].filter(Boolean),
        ),
      ),
  ...
  const filterTabs = useMemo(
    () => [
      ...
      ...filterCategoryOptions.map((category) => ({
        label: category,
        value: toCategoryTabValue(category),
      })),
    ],
    [filterCategoryOptions],
  );
  const activeTab = filterTabs.some((tab) => tab.value === requestedTab) ? requestedTab : 'all';
  ...
  const filteredCases = cases.filter((item) => {
    ...
    const itemTabValue = normalizedCategory
      ? toCategoryTabValue(normalizedCategory)
    ...
    const matchesTab =
      activeTab === 'all' || itemTabValue === activeTab;
  ...
  const totalPages = Math.max(1, Math.ceil(orderedFilteredCases.length / CASES_PER_PAGE));
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const startIndex = (safeCurrentPage - 1) * CASES_PER_PAGE;
  const pagedCases = orderedFilteredCases.slice(startIndex, startIndex + CASES_PER_PAGE);
  ...
  useEffect(() => {
    ...
    if (preloadedCategories) {
      setManagedCategories(preloadedCategories);
    }
    ...
    const loadCases = async () => {
      try {
        ...
        setManagedCategories(Array.isArray(data.categories) ? data.categories : []);
      } catch (error) {
  ...
  }, [preloadedCases, preloadedCategories]);
  ...
  const openCreateEditor = () => {
    setCreateDraft({
      title: '',
      category: filterCategoryOptions[0] || DEFAULT_INSTALLATION_CASE_CATEGORY,
      subtitle: '',
